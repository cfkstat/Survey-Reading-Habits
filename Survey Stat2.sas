...
/*		if question_name >= "19" and question_name <= "22";*/
		if type = "实地民调" then score = .;
	RUN;

	PROC tabulate data=&ds._1(where=(question_name < "19"));
		class "&type."n value_text question_name;
		var count;
...
/*		table question_name="", "&type."n="",  value_text=""*count=""*(ROWPCTSUM=""*f=pctfmt.);*/
	...
	PROC sql;
		...
		select *,sum(score)/sum(count) as "评分"n 
			from &ds._1 
			where question_name ~=" " and question_name <= "19" and type = "网络民调";
			group by "&type."n, question_name;
	QUIT; 
	PROC tabulate data=tmp;
		class "&type."n value_text question_name ;
		var count "评分"n;
		table question_name="", "&type."n=""  all="小计",  ((value_text="" )*count=""*(sum="" ROWPCTSUM=""*f=pctfmt.)) "评分"n*mean="";
/*		table question_name="", "&type."n="",  ((value_text="" )*count=""*(ROWPCTSUM=""*f=pctfmt.)) "评分"n*mean="";*/
	RUN;
%MEND state2;
*************************************广州********************************;
/*%let ds =A1;*/
...
%MACRO outXls2(file, ds, city);
	ods  tagsets.excelxp   file="&file." options(sheet_name="性别" sheet_interval='none')  style=analysis;
	%state2(性别, &ds, &city.);
	ods  tagsets.excelxp  options(sheet_name="学历" sheet_interval='none')   style=analysis;
	%state2(学历, &ds, &city.);
	ods  tagsets.excelxp  options(sheet_name="年龄" sheet_interval='none')   style=analysis;
	%state2(年龄, &ds, &city.);
    ods  tagsets.excelxp  options(sheet_name="收入" sheet_interval='none')   style=analysis;
	%state2(收入, &ds, &city.);
	ods  tagsets.excelxp  options(sheet_name="户口" sheet_interval='none')   style=analysis;
	%state2(户口, &ds, &city.);
	ods  tagsets.excelxp  options(sheet_name="岗位类型" sheet_interval='none') style=analysis;
	%state2(岗位类型, &ds, &city.);
	ods  tagsets.excelxp  options(sheet_name="职业身份" sheet_interval='none')   style=analysis;
	%state2(职业身份, &ds, &city.);
